...
export default function ProductQuestions({ productId }: ProductQuestionsProps) {
  ...
  const fetchQuestions = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/questions?productId=${productId}&filter=${filter}`)
      if (response.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <h3 className="text-xl font-bold">Product Questions ({questions.length})</h3>
          <div className="flex space-x-2">
            ...
            <Button
              ...
              className={filter === 'unanswered' ? 'bg-blue-600' : ''}
            >
              Unanswered
            </Button>
          </div>
      ...
      {questions.length === 0 ? (
        <div className="text-center py-12 border rounded-lg">
          ...
          <p className="text-sm text-gray-400">Be the first to ask about this product</p>
        </div>
      ) : (
        <div className="space-y-4">
          {questions.map((q) => (
            <div key={q.id} className="border rounded-lg p-4 space-y-3">
              ...
              {q.answer && (
                <div className="ml-11 pl-4 border-l-2 border-blue-200">
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-1">
                        <span className="font-medium text-sm text-green-600">Seller</span>
                        {q.answeredAt && (
              ...
              {!q.answer && (
                <div className="ml-11">
                  <span className="text-xs text-gray-500 italic">Waiting for seller's answer...</span>
                </div>
              )}
            </div>
          ))}
